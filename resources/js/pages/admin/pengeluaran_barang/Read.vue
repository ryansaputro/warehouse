          ...
          <div>
                <vue-bootstrap4-table 
                    ...
                    <template slot="action-slot" slot-scope="props" v-if="$can('edit-divisi')">
                        ...
                        <button type="button" class="btn btn-info btn-sm" v-if="props.row.status_posting == '1'" @click="onPosting(props.row)" v-tooltip="'Posting dapat dilakukan jika detail barang sudah terisi.'"><i class="fa fa-send" aria-hidden="true"></i> Posting</button>
                    </template>
                </vue-bootstrap4-table>
                <!--   -->
                <vue-bootstrap4-table 
...
export default {
    ...
    methods: {
        onNoEdit(row) {
            this.$swal('Maaf', 'Tidak dapat meng-edit <br>No Penerimaan: <b>'+row.no_pengeluaran+'</b> dikarenakan telah diposting', 'error');
        },
        onShow(row) {
            this.$router.push("/pengeluaran_barang/"+row.no_pengeluaran+"/show");
        },
        onEdit(row) {
            this.$router.push("/pengeluaran_barang/"+row.no_pengeluaran+"/edit");
        },
        onDelete(row) {
            axios.post("pengeluaran_barang/deletePengeluaran", {
              no_pengeluaran: row.no_pengeluaran,  
            })
            .then(response => {
                this.getProjects();
                // push router ke read data
                this.$swal('Berhasil', 'Pengeluaran Barang berhasil dihapus', 'success');
            })
            .catch(errors => {
        ...
        onPosting(row) {
            ...
            .catch(errors => {
                ...
                if (errors.response) {
                    this.$swal({
                      title: "Opps",
                      text: "Maaf data item belum lengkap",
                      type: "error"
                    }).then(function() {
                        this.getProjects();
                    });
        ...
        newData(payload) {
                this.$router.push("/pengeluaran_barang/create");
        },
        getProjects() {
            this.loading = true
            axios.get('pengeluaran_barang/index')
                .then(response => {
