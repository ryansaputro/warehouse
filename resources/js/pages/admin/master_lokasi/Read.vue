          ...
          <div class="row">
            ...
            <div class="col-md-6">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <router-link v-if="$can('read-lokasi')" class="btn btn-primary w-100" to="lokasi/create">+ Tambah</router-link>
                    </div>
                </div>
            </div>
          </div>
        ...
        <datatable :columns="columns" :sortKey="sortKey" :sortOrders="sortOrders" @sort="sortBy">
            <tbody>
                <tr v-for="(project, index) in paginated" :key="project.id">
                    ...
                    <td>
                      <router-link v-if="$can('edit-lokasi')" class="btn btn-primary btn-xs" :to="'/lokasi/'+project.id">Edit</router-link>
                      <button v-if="$can('delete-lokasi')" class="btn btn-danger btn-xs" v-on:click="deleteData(project.id)">Delete</button>
                    </td>
                </tr>
...
export default {
    ...
    data() {
        let sortOrders = {};
        let columns = [
            {label: 'Nama Gerbang', name: 'nama_gerbang'},
    ...
    methods: {
      doMath: function (index) {
        ...
        getProjects() {
            this.loading = true
            axios.get('lokasi', {params: this.tableData})
                .then(response => {
        ...
        // delete data
          this.loading = true
          axios.delete("lokasi/" + id).then(response => {
            this.getProjects();
            // $swal function calls SweetAlert into the application with the specified configuration.
            this.$swal('Hapus', 'Data Lokasi Berhasil dihapus.', 'success');
          }).finally(() => {
                this.loading =  false
          });
        },
        ...
        filteredProjects() {
            ...
            if (sortKey) {
                projects = projects.slice().sort((a, b) => {
                    let index = this.getIndex(this.columns, 'nama_gerbang', sortKey);
