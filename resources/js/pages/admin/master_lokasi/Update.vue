...
export default {
  ...
  methods: {
    loadData() {
      ...
      axios
        .get("lokasi/" + this.$route.params.id)
        .then(response => {
          // post value yang dari response ke form
          this.form.nama_gerbang = response.data[0].nama_gerbang;
    ...
    updateData() {
      ...
      axios
        .put("lokasi/" + this.$route.params.id, {
        ...
        .then(response => {
          ...
          this.$swal('Berhasil', 'Data Lokasi Berhasil diperbarui', 'success');
        })
        .catch(errors => {
            if (errors.response) {
                var data = '';
